...
@client.command(name='google',
                description="It hopefully Googles stuff and gives you the URL",
                brief="Googles Stuff",
                pass_context=True)
async def googler(context):
    url_list = ""
    counter = 0
    for j in search(context.message.content[6:], tld="co.in", num=10, stop=5, pause=2):
        counter += 1
        if counter == 1:
            url_list += f"Well there is this one,\n         <{j}>"
        else:
            url_list += f"\nand this one,\n         <{j}>"
    await client.say(url_list)
